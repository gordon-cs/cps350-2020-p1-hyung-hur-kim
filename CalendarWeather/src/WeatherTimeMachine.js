 ...
 */
class WeatherTimeMachine extends Component {
  constructor(props) {
  ...
  async getWeatherApi() {
    ...
    try {
      let response = await fetch(darkskyURL + "/" + ApiKey + "/" + latLon + "," + time);
      ...
      this.setState({
        isLoading: false,
        weatherTimeMachine: responseJson,
        tempScale: tempScale,
      });
    } catch (error) {
      console.error(error);
  ...
  render() {
    if (this.state.isLoading) {
      ...
      // Show data from API
      return (
		  <WeatherApi weatherTimeMachine={this.state.weatherTimeMachine}> </WeatherApi>
      );
    }
  }
}

export default WeatherTimeMachine;
